...
def random_descending_list(n: int) -> list[int]:
    ...
    new_list: list[int] = [MAX_VAL] #maximum value is the initial element
    for x in range(1, n):
        new_element = randint(new_list[-1]- MAX_VAL // n, new_list[-1])
        new_list.append(new_element) #add to the list
    return new_list
